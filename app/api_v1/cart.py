...
from ..forms.form import (CartItemForm, CheckoutForm, OrderConfirmationForm,
                          OrderForm, OrderItemForm, RemoveItemForm,
                          UpdateQuantityForm)
...
def view_cart():
    ...
    return render_template(
        "view_cart.html", cart=cart, total_amount=total_amount, user=current_user
    )
...
def checkout():
    ...
    if form.validate_on_submit():
        ...
        if current_user.cart.items:
            ...
            flash(
                "Your cart is empty. Please add items before checking out.", "warning"
            )
...
def create_order():
    ...
    return render_template(
        "create_order.html", order_form=order_form, order_item_form=order_item_form
    )
